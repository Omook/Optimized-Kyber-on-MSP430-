...
void poly_pointwise_montgomery(poly *c, const poly *a, const poly *b) {
    ...
    //asm_poly_pointwise_mont(c->coeffs, a->coeffs, b->coeffs);
...
void poly_decompose_a1(poly *a1, const poly *a) {
    int32_t t;

    for (size_t i = 0; i < N; ++i) {
        a1->coeffs[i] = decompose(&t, a->coeffs[i]);
    }

}

void poly_decompose_a0(poly *a0, const poly *a) {

    for (size_t i = 0; i < N; ++i) {
        decompose(&a0->coeffs[i], a->coeffs[i]);
    }
...
void polyt0_pack(uint8_t *r, const poly *a) {
    ...
    for (i = 0; i < N / 8; ++i) {
        t[0] = (1ULL << (D - 1)) - a->coeffs[8 * i + 0];
        t[1] = (1ULL << (D - 1)) - a->coeffs[8 * i + 1];
        t[2] = (1ULL << (D - 1)) - a->coeffs[8 * i + 2];
        t[3] = (1ULL << (D - 1)) - a->coeffs[8 * i + 3];
        t[4] = (1ULL << (D - 1)) - a->coeffs[8 * i + 4];
        t[5] = (1ULL << (D - 1)) - a->coeffs[8 * i + 5];
        t[6] = (1ULL << (D - 1)) - a->coeffs[8 * i + 6];
        t[7] = (1ULL << (D - 1)) - a->coeffs[8 * i + 7];
...
void polyt0_unpack(poly *r, const uint8_t *a) {
    ...
    for (i = 0; i < N / 8; ++i) {
        ...
        r->coeffs[8 * i + 0] = (1ULL << (D - 1)) - r->coeffs[8 * i + 0];
        r->coeffs[8 * i + 1] = (1ULL << (D - 1)) - r->coeffs[8 * i + 1];
        r->coeffs[8 * i + 2] = (1ULL << (D - 1)) - r->coeffs[8 * i + 2];
        r->coeffs[8 * i + 3] = (1ULL << (D - 1)) - r->coeffs[8 * i + 3];
        r->coeffs[8 * i + 4] = (1ULL << (D - 1)) - r->coeffs[8 * i + 4];
        r->coeffs[8 * i + 5] = (1ULL << (D - 1)) - r->coeffs[8 * i + 5];
        r->coeffs[8 * i + 6] = (1ULL << (D - 1)) - r->coeffs[8 * i + 6];
        r->coeffs[8 * i + 7] = (1ULL << (D - 1)) - r->coeffs[8 * i + 7];
    }
