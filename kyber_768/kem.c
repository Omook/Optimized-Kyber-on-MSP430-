...
int PQCLEAN_KYBER768_CLEAN_crypto_kem_keypair(uint8_t *pk,
    ...
    hash_h(sk + KYBER_SECRETKEYBYTES - 2 * KYBER_SYMBYTES, pk, KYBER_PUBLICKEYBYTES); // 826, 170
...
int PQCLEAN_KYBER768_CLEAN_crypto_kem_enc(uint8_t *ct,
    ...
    hash_h(buf, buf, KYBER_SYMBYTES); //95,735

    /* Multitarget countermeasure for coins + contributory KEM */
    hash_h(buf + KYBER_SYMBYTES, pk, KYBER_PUBLICKEYBYTES); // 826,169
    hash_g(kr, buf, 2 * KYBER_SYMBYTES); // 92,992
