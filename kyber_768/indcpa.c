...
void PQCLEAN_KYBER768_CLEAN_indcpa_keypair(uint8_t pk[KYBER_INDCPA_PUBLICKEYBYTES],
    ...
    hash_g(buf, buf, KYBER_SYMBYTES); // 92,824

    gen_a(a, publicseed); // 2,587,135
    ...
    PQCLEAN_KYBER768_CLEAN_polyvec_ntt(&skpv);   //306.755
    PQCLEAN_KYBER768_CLEAN_polyvec_ntt(&e);      //306.755
    ...
    PQCLEAN_KYBER768_CLEAN_polyvec_add(&pkpv, &pkpv, &e); // 6244
    PQCLEAN_KYBER768_CLEAN_polyvec_reduce(&pkpv); // 64,607

    pack_sk(sk, &skpv); //16,611
    pack_pk(pk, &pkpv, publicseed); //16,776
}
...
void PQCLEAN_KYBER768_CLEAN_indcpa_enc(uint8_t c[KYBER_INDCPA_BYTES],
    ...
    unpack_pk(&pkpv, seed, pk);  //13,122
    PQCLEAN_KYBER768_CLEAN_poly_frommsg(&k, m); //3197
    gen_at(at, seed); // 2,587,069
    ...
    PQCLEAN_KYBER768_CLEAN_poly_getnoise_eta2(&epp, coins, nonce++); //830,884

    PQCLEAN_KYBER768_CLEAN_polyvec_ntt(&sp); // 306,755

    // matrix-vector multiplication
    for (i = 0; i < KYBER_K; i++) {
        PQCLEAN_KYBER768_CLEAN_polyvec_basemul_acc_montgomery(&b.vec[i], &at[i], &sp);
    }
    PQCLEAN_KYBER768_CLEAN_polyvec_basemul_acc_montgomery(&v, &pkpv, &sp); // 726,123

    PQCLEAN_KYBER768_CLEAN_polyvec_invntt_tomont(&b); //527,531
